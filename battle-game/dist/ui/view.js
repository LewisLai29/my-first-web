...
const required = (root, selector) => {
    ...
    if (!element)
        throw new Error(`Wordfront element is missing: ${selector}`);
    return element;
};
const formatSeconds = (milliseconds) => `${(Math.max(0, milliseconds) / 1000).toFixed(1)}s`;
export class DomWordfrontView {
    root;
    questionDurationMs;
    entriesById;
    animations;
    ...
    lastQuestionId = '';
    lastWaveKey = '';
    lastResultRevision = -1;
    constructor(root, entries, handlers, questionDurationMs) {
        this.root = root;
    ...
    render(state, now) {
        ...
        const game = required(this.root, '.wordfront-game');
    ...
    play(effect) {
        if (effect.type === 'ANNOUNCE') {
            required(this.root, '[data-wordfront-status]').textContent = effect.message;
            return Promise.resolve();
        }
        if (effect.type === 'FOCUS') {
            window.requestAnimationFrame(() => {
                if (effect.target === 'first-answer') {
                    this.root.querySelector('[data-wordfront-action="answer"]:not(:disabled)')?.focus();
                }
                else if (effect.target === 'resume') {
                    this.root.querySelector('[data-wordfront-action="resume"]')?.focus();
                }
                else {
                    this.root.querySelector('[data-wordfront-action="restart"]')?.focus();
                }
            });
            return Promise.resolve();
        }
    ...
    renderWave(wave) {
        ...
        const container = required(this.root, '[data-wordfront-enemies]');
        ...
        wave.enemies.forEach((enemy, index) => {
            const element = container.querySelector(`[data-enemy-id="${enemy.id}"]`);
            if (!element)
                return;
    ...
    renderQuestion(state, now, effectivePhase) {
        const panel = required(this.root, '[data-wordfront-question-panel]');
        const definition = required(this.root, '[data-wordfront-definition]');
        const answers = required(this.root, '[data-wordfront-answers]');
        ...
        if (this.lastQuestionId !== state.question.id) {
            ...
            answers.replaceChildren(...state.question.options.map((option, index) => {
                ...
                required(button, 'strong').textContent = option.label;
                return button;
            }));
        }
        const inPenalty = state.penaltyDeadlineGameMs !== null && now < state.penaltyDeadlineGameMs;
        const penalty = required(this.root, '[data-wordfront-penalty]');
        penalty.hidden = !inPenalty;
        penalty.textContent = inPenalty ? `Try again in ${Math.ceil((state.penaltyDeadlineGameMs - now) / 1000)}s` : '';
        answers.querySelectorAll('[data-entry-id]').forEach((button) => {
        ...
        const timer = required(this.root, '[data-wordfront-timer]');
        const timerBar = required(this.root, '[data-wordfront-timer-bar]');
    ...
    renderScreens(state, now, effectivePhase) {
        const intro = required(this.root, '[data-wordfront-intro]');
        const pause = required(this.root, '[data-wordfront-pause]');
        const waveClear = required(this.root, '[data-wordfront-wave-clear]');
        const result = required(this.root, '[data-wordfront-result]');
        const rotate = required(this.root, '[data-wordfront-rotate]');
        ...
        if (state.phase === 'paused' && !portrait) {
            ...
            const resume = required(pause, '[data-wordfront-action="resume"]');
            resume.disabled = blockers.length > 0;
            required(pause, '[data-wordfront-pause-message]').textContent = blockers.length > 0
        ...
        if (effectivePhase === 'wave-transition') {
            ...
            required(waveClear, '[data-wordfront-next-wave]').textContent = `Wave ${state.currentWaveIndex + 2} in ${Math.max(1, Math.ceil(remaining / 1000))}`;
        }
    ...
    renderResult(state, phase) {
        ...
        required(this.root, '[data-wordfront-result-kicker]').textContent = phase === 'victory' ? 'Battle complete' : 'The line has fallen';
        required(this.root, '[data-wordfront-result-title]').textContent = phase === 'victory' ? 'Victory!' : 'Defeat';
        required(this.root, '[data-wordfront-result-stats]').innerHTML = `
        ...
        const review = required(this.root, '[data-wordfront-review]');
