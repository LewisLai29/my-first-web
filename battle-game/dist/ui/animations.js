...
export class WordfrontAnimations {
    ...
    async play(effect) {
        if (effect.type === 'ANIMATE_PLAYER_ATTACK')
            return this.playerAttack(effect.word, effect.enemyId);
        if (effect.type === 'ANIMATE_ENEMY_ATTACK')
            return this.enemyAttack(effect.enemyId);
        if (effect.type === 'ANIMATE_ENEMY_DEATH')
            return this.enemyDeath(effect.enemyId);
    }
    destroy() {
    ...
    async playerAttack(word, enemyId) {
        const battlefield = this.root.querySelector('[data-wordfront-battlefield]');
        const player = this.root.querySelector('[data-wordfront-player]');
        const enemy = this.root.querySelector(`[data-enemy-id="${enemyId}"]`);
        if (!battlefield || !player || !enemy)
            return;
    ...
    async playPlayerFrames(player) {
        const baseImage = player.querySelector('img');
        if (!baseImage)
            return;
        ...
        const renderFrame = () => {
            if (driver.playState === 'idle' || driver.playState === 'finished')
                return;
            ...
            if (frameSource && baseImage.src !== frameSource)
                baseImage.src = frameSource;
    ...
    async enemyAttack(enemyId) {
        const enemy = this.root.querySelector(`[data-enemy-id="${enemyId}"]`);
        const player = this.root.querySelector('[data-wordfront-player]');
        const tasks = [];
        if (enemy)
            tasks.push(this.animate(enemy, [
                { translate: '0 0' },
                { translate: '-28px 0', offset: 0.45 },
                { translate: '0 0' },
            ], { duration: 460, easing: 'ease-in-out' }));
        if (player)
            tasks.push(this.animate(player, [
