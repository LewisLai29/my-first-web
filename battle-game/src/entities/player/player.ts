import {
    PLAYER_BASE_ATTACK,
    PLAYER_DEFENSE,
    PLAYER_MAX_ATTACK_BONUS,
    PLAYER_MAX_HP,
    PLAYER_STREAK_THRESHOLD,
...
export function attackForStreak(correctStreak: number): number {
...
export function addCorrectAnswer(player: PlayerState): PlayerState {
    const correctStreak = player.correctStreak + 1;
    return { ...player, correctStreak, attack: attackForStreak(correctStreak) };
}

export function resetCorrectStreak(player: PlayerState): PlayerState {
