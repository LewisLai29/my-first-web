...
`config.ts` 匯出唯讀設定，避免在 reducer 內散落 magic numbers：
...
export const DEFAULT_CONFIG: WordfrontConfig = {
    ...
    waves: [
        { enemies: [{ kind: 'normal', count: 3, hp: 2, damage: 10 }] },
        { enemies: [{ kind: 'strong', count: 3, hp: 3, damage: 15 }] },
        {
            enemies: [
                { kind: 'strong', count: 3, hp: 3, damage: 15 },
                { kind: 'boss', count: 1, hp: 5, damage: 20 },
            ],
        },
    ],
};
...
- reducer 先套用 1 點傷害並進入 `resolving-player-attack`。
...
- 正規化詞性集合。
- 確認至少有 29 個可作為目標的唯一單字；不足時顯示 load error，不啟動遊戲。
...
每局開始時先抽出 29 個不重複 target，順序固定於該局，避免重新 render 或 pause 改變後續題目。每題建立選項的順序為：
...
| 題庫格式錯誤或少於 29 個 target | 視為 load error，不建立部分遊戲 |
