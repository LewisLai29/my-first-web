...
1. 在 10 秒內選出正確的英文單字。
2. 答對時，玩家會依攻擊與怪物防禦造成傷害；連續答對第 3 題起會提升攻擊。
3. 答錯時，錯誤選項會被排除，並暫時鎖定作答 3 秒。
4. 超過 10 秒沒有答對，怪物就會攻擊玩家。
5. 清除三個波次的怪物後獲勝。
6. 玩家 HP 降到 0 時失敗。

目前一局共有 10 隻怪物，每局會預備最多 38 道題目；完美連擊約 21 題通關。
...
每局會隨機選出最多 38 個不重複的目標單字。每題有一個正確答案和三個干擾選項。

干擾選項會優先選擇相同詞性的單字；不足三個時，才從其他合適的單字補足。題庫至少需要 38 筆有效單字，而且必須能為每題找到三個不重複、沒有歧義的選項。
...
| 波次 | 怪物 | 總 HP |
|---|---|---:|
| Wave 1 | 3 隻 Normal | 24 |
| Wave 2 | 3 隻 Strong | 36 |
| Wave 3 | 3 隻 Strong、1 隻 Boss | 51 |
| 合計 | 10 隻怪物 | 111 |
